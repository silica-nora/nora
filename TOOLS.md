...
- 全时段硬规则（早报/午报/晚报）：必须先读数据并给出可读解读，不把“自己去看链接”丢给 tk

### 新闻去重台账（2026-04-05 新增，强制）
- **所有已推送新闻必须落盘**到：`memory/news-push-history.tsv`
- 记录字段（TSV）：`pushed_at\tperiod\ttitle\turl\thash\tsource`
- `hash` 规则：`title + "|" + url` 归一化后生成（用于稳定去重）
- 推送前先查台账：命中同 `hash` 则视为已推送，直接跳过
- 去重窗口默认 48 小时；若同链接重复高频出现，仍默认不重复推送
- 推送成功后立刻写入台账，确保后续轮次可去重
...
### A股样本不足强制优化协议（2026-03-27 新增）
- 触发词：一旦准备输出“可用样本不足/低置信度”
- **禁止直接结束**，必须先执行以下优化动作再输出：
  1) 立刻补抓至少 3 个权威源（交易所/中证指数/东方财富/新浪行情/财联社）
  2) 重写查询词（加日期、加“收盘/午盘/成交额/涨跌家数/北向资金”等结构化关键词）
  3) 给出“已执行的优化动作清单”+“仍缺失的数据项”
  4) 若仍不足，只允许给一次低置信结论，并附下一步修复计划（不是重复道歉）
- **硬约束**：连续两次出现“样本不足”，下一轮必须优先做流程修复（脚本/数据源/模板），再发分析。
